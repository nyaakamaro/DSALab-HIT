...
// ============ COMMENT: bình luận / đánh giá sách (1 danh sách liên kết cho mỗi cuốn sách) ============
struct CommentNode {
    std::string ma_doc_gia;
    std::string ho_ten_doc_gia;
    std::string noi_dung;
    int so_sao;            // đánh giá 1-5 sao
    std::string thoi_gian;
    CommentNode* next;

    CommentNode(const std::string& ma, const std::string& ten, const std::string& nd,
                int sao, const std::string& tg)
        : ma_doc_gia(ma), ho_ten_doc_gia(ten), noi_dung(nd), so_sao(sao),
          thoi_gian(tg), next(nullptr) {}
};
...
public:
    CommentList();
    ~CommentList();
    CommentList(const CommentList&) = delete;            // Book chứa CommentList theo con trỏ cây,
    CommentList& operator=(const CommentList&) = delete;  // không cần copy -> tránh double free

    void push(const std::string& ma_doc_gia, const std::string& ho_ten,
              const std::string& noi_dung, int so_sao, const std::string& thoi_gian);
    bool isEmpty() const;
    int size() const;
    double diemTrungBinh() const;                 // điểm đánh giá trung bình (0 nếu chưa có)
    std::vector<CommentNode*> toVector() const;    // để in / lưu file
};
...
struct Book {
    ...
    std::string tom_tat;         // tóm tắt nội dung sách
    std::string noi_dung_online; // link đọc online (http...) hoặc nội dung đầy đủ dạng text

    WaitQueue hang_cho;   // queue chờ mượn riêng cho sách này
    CommentList binh_luan; // danh sách bình luận / đánh giá riêng cho sách này
...
public:
    ...
    bool capNhatTomTatVaNoiDung(const std::string& ma, const std::string& tom_tat,
                                 const std::string& noi_dung_online);
    void xemTomTat(const std::string& ma) const;
    void docNoiDungOnline(const std::string& ma, bool tuMoTrinhDuyet = false) const;
    bool themBinhLuan(const std::string& ma_sach, const std::string& ma_doc_gia,
                       const std::string& noi_dung, int so_sao);
    void xemBinhLuan(const std::string& ma_sach) const;
    void xemChiTietSach(const std::string& ma) const; // tóm tắt + link online + điểm đánh giá TB
